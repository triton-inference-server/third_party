...
template <typename T> __global__ void tinyKernel(T *d_a, int numElem) {
...
struct _24ByteStruct {
  ...
  __host__ __device__ void operator+=(int other) {
    a += other;
    b += other;
    c += other;
  }
...
template <typename T, int expectedSize> void testAlign() {
  ...
  ASSERT_EQ(CNMEM_STATUS_SUCCESS,
            cnmemMalloc((void **)&ptr0, size, streams[0]));
  ASSERT_EQ(CNMEM_STATUS_SUCCESS,
            cnmemMalloc((void **)&ptr1, size, streams[1]));

  ASSERT_EQ(cudaSuccess, cudaMemcpyAsync(ptr0, cpuData, size,
                                         cudaMemcpyHostToDevice, streams[0]));
  ASSERT_EQ(cudaSuccess, cudaMemcpyAsync(ptr1, cpuData, size,
...
TEST_F(CnmemTest, alignment8) { testAlign<char, 1>(); }

TEST_F(CnmemTest, alignment16) { testAlign<short, 2>(); }

TEST_F(CnmemTest, alignment32) { testAlign<float, 4>(); }

TEST_F(CnmemTest, alignment64) { testAlign<double, 8>(); }

TEST_F(CnmemTest, alignment192) { testAlign<_24ByteStruct, 24>(); }
