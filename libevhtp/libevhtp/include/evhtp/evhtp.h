...
typedef void (*evhtp_hook_event_cb)(evhtp_connection_t *conn, short events,
                                    void *arg);
...
typedef evhtp_res (*evhtp_hook_chunk_new_cb)(evhtp_request_t *r, uint64_t len,
                                             void *arg);
...
typedef int (*evhtp_ssl_chk_issued_cb)(evhtp_x509_store_ctx_t *ctx,
                                       evhtp_x509_t *x, evhtp_x509_t *issuer);
typedef EVP_PKEY *(*evhtp_ssl_decrypt_cb)(char *privfile);

typedef int (*evhtp_ssl_scache_add)(evhtp_connection_t *connection,
                                    evhtp_ssl_data_t *sid, int sid_len,
                                    evhtp_ssl_sess_t *sess);
typedef void (*evhtp_ssl_scache_del)(evhtp_t *htp, evhtp_ssl_data_t *sid,
                                     int sid_len);
typedef evhtp_ssl_sess_t *(*evhtp_ssl_scache_get)(
    evhtp_connection_t *connection, evhtp_ssl_data_t *sid, int sid_len);
...
#define EVHTP_FLAG_ENABLE_ALL                                                  \
  ...
  | EVHTP_FLAG_ENABLE_REUSEPORT | EVHTP_FLAG_ENABLE_NODELAY |                  \
      EVHTP_FLAG_ENABLE_DEFER_ACCEPT

  uint16_t flags; /**< the base flags set for this context, see: EVHTP_FLAG_* */
  uint16_t parser_flags; /**< default query flags to alter 'strictness' (see
...
#define EVHTP_CONN_FLAG_OWNER                                                  \
  (1 << 2) /**< set to 1 if this structure owns the bufferevent */
#define EVHTP_CONN_FLAG_VHOST_VIA_SNI                                          \
  (1 << 3) /**< set to 1 if the vhost was found via SSL SNI */
#define EVHTP_CONN_FLAG_PAUSED                                                 \
  (1 << 4) /**< this connection has been marked as paused */
#define EVHTP_CONN_FLAG_CONNECTED                                              \
  (1 << 5) /**< client specific - set after successful connection */
#define EVHTP_CONN_FLAG_WAITING                                                \
  (1 << 6) /**< used to make sure resuming  happens AFTER sending a reply */
#define EVHTP_CONN_FLAG_FREE_CONN (1 << 7)
#define EVHTP_CONN_FLAG_KEEPALIVE                                              \
  (1 << 8) /**< set to 1 after the first request has been processed and the    \
 ...
 */
EVHTP_EXPORT void evhtp_set_timeouts(evhtp_t *htp, const struct timeval *r,
...
EVHTP_EXPORT
WINDEPRECATED(
    "evhtp_disable_100 will soon be deprecated, use htp->flags instead")
void evhtp_disable_100_continue(evhtp_t *htp) DEPRECATED(
    "evhtp_disable_100 will soon be deprecated, use htp->flags instead");
 ...
 */
EVHTP_EXPORT void evhtp_set_gencb(evhtp_t *htp, evhtp_callback_cb cb,
                                  void *arg);
 ...
 * @return
 */
EVHTP_EXPORT void evhtp_set_pre_accept_cb(evhtp_t *htp, evhtp_pre_accept_cb,
                                          void *arg);
 ...
 * @return
 */
EVHTP_EXPORT void evhtp_set_post_accept_cb(evhtp_t *htp, evhtp_post_accept_cb,
                                           void *arg);
...
/**
 * @brief attempts to find the callback matching the exact string 'needle'. This
 * is useful in cases where we want to get the original handle, but is not in
 * scope.
 *
 *        with pattern based callbacks, this does not attempt to find a callback
 * that would match the string if the pattern matcher was executed.
 *
 *        Meaning:
...
EVHTP_EXPORT int evhtp_connection_set_hook(evhtp_connection_t *c,
                                           evhtp_hook_type type, evhtp_hook cb,
                                           void *arg);
 ...
 * @return
 */
EVHTP_EXPORT int evhtp_bind_socket(evhtp_t *htp, const char *addr,
 ...
 */
EVHTP_EXPORT int evhtp_accept_socket(evhtp_t *htp, evutil_socket_t sock,
                                     int backlog);
 ...
 */
EVHTP_EXPORT int evhtp_use_threads_wexit(evhtp_t *, evhtp_thread_init_cb,
                                         evhtp_thread_exit_cb, int nthreads,
                                         void *arg);
 ...
 * @return
 */
EVHTP_EXPORT int evhtp_add_vhost(evhtp_t *evhtp, const char *name,
                                 evhtp_t *vhost);
 ...
 */
EVHTP_EXPORT evhtp_kv_t *evhtp_kv_new(const char *key, const char *val,
 ...
 * @return
 */
EVHTP_EXPORT int evhtp_kvs_for_each(evhtp_kvs_t *kvs, evhtp_kvs_iterator cb,
                                    void *arg);
 ...
 * @return 0 on success, -1 on error
 */
EVHTP_EXPORT int evhtp_unescape_string(unsigned char **out, unsigned char *str,
                                       size_t str_len);
 ...
 */
EVHTP_EXPORT struct bufferevent *
evhtp_connection_get_bev(evhtp_connection_t *conn);
...
#ifndef EVHTP_DISABLE_SSL
EVHTP_EXPORT evhtp_connection_t *
evhtp_connection_ssl_new(struct event_base *evbase, const char *addr,
                         uint16_t port, evhtp_ssl_ctx_t *ctx);
#endif
 ...
 */
EVHTP_EXPORT int evhtp_make_request(evhtp_connection_t *c, evhtp_request_t *r,
