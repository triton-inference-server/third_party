...
#define log_debug(M, ...)                                                      \
  fprintf(stderr,                                                              \
          __log_debug_color("DEBUG") " " __log_func_color("%s/%s:%-9d")        \
              __log_args_color(M) "\n",                                        \
...
#define log_error(M, ...)                                                      \
  fprintf(                                                                     \
      stderr,                                                                  \
      __log_error_color("ERROR") " " __log_func_color("%s:%-9d")               \
          __log_args_color(M) " :: " __log_errno_color("(errno: %s)") "\n",    \
      __FILENAME__, __LINE__, ##__VA_ARGS__, clean_errno())

#define log_warn(M, ...)                                                       \
  fprintf(                                                                     \
      stderr,                                                                  \
      __log_warn_color("WARN") "  " __log_func_color("%s:%-9d")                \
          __log_args_color(M) " :: " __log_errno_color("(errno: %s)") "\n",    \
      __FILENAME__, __LINE__, ##__VA_ARGS__, clean_errno())

#define log_info(M, ...)                                                       \
  fprintf(stderr,                                                              \
          __log_info_color("INFO") "  " __log_func_color("%4s:%-9d")           \
              __log_args_color(M) "\n",                                        \
